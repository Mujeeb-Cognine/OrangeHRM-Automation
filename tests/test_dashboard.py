...
class TestDashboard(BaseClass):
    ...
    def test_verifying_card_in_dashboard(self):
        ...
        BasePageFragments().verify_element_presence(element_locator=DashboardPage().dashboard_cards)

    def test_change_employee_on_leave_today_config(self):

        # Check if the 'emp_leave_card' is present
        if not BasePageFragments().verify_element_presence( element_locator=DashboardPage().emp_leave_card):
            retry_count = 0
            max_retries = 3
        ...
        # Verifying the dashboard cards are present in the Dashboard Page.
        BasePageFragments().verify_element_presence(element_locator=DashboardPage().dashboard_cards)
        BasePageFragments().verify_element_presence(element_locator=DashboardPage().gear_icon)
        # Clicking on employee leave card gear icon.
        DashboardPage().emp_leave_card_gear_btn().click()
        BasePageFragments().wait_until_orng_hrm_dialog_appears()
        # Clicking on toggle button in the dialog.
        BasePageFragments().click_toggle_switch()
        # Clicking on save button in the dialog.
        BasePageFragments().click_orng_hrm_dialog_save()
